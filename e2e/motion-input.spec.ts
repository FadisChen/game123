...
test("switching an existing first-person player to motion hides the canvas and restores it in main mode", async ({ browser }) => {
  ...
  try {
    ...
    await expect(player.locator(".game-canvas")).toBeHidden();
    await player.screenshot({ path: "artifacts/ui-previews/motion-player.png" });
    await player.setViewportSize({ width: 390, height: 844 });
    await expect(player.locator(".landscape-guard")).toBeHidden();
    await player.screenshot({ path: "artifacts/ui-previews/motion-player-portrait.png" });
...
test("motion mode turns one vertical shake into alternating steps", async ({ browser }) => {
  ...
  try {
    ...
    await expect(player.locator("#app")).toHaveCSS("background-color", "rgb(0, 0, 0)");
    await expect(player.locator(".player-hud")).toBeHidden();
    await expect(player.locator(".motion-prompt")).toContainText("請看主辦方畫面");
    await player.setViewportSize({ width: 390, height: 844 });
    await expect(player.locator(".landscape-guard")).toBeHidden();
...
test("motion mode keeps the black guidance screen when the sensor never reports data", async ({ browser }) => {
  const hostContext = await browser.newContext();
  const playerContext = await browser.newContext({ viewport: { width: 900, height: 420 } });
  const host = await hostContext.newPage();
  const player = await playerContext.newPage();
  ...
  try {
    ...
    await expect(player.locator(".foot-button--left")).toBeHidden();
    await expect(player.locator(".step-hint")).toHaveText("感應器無法使用，請檢查動作感應權限或洽主辦方");
    await expect(player.locator(".game-canvas")).toHaveCount(0);
    await expect(player.locator(".motion-prompt")).toBeVisible();
  } finally {
    await hostContext.close();
