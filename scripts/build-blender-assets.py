    ...
    if name == "player":
        ...
        for obj in objects:
            ...
            for color in colors.data:
                color.color = obj.data.materials[0].diffuse_color
        bpy.ops.object.select_all(action="SELECT")
...
reset()
skin = material("warm porcelain", (0.76, 0.48, 0.29), 0.55)
hair = material("espresso hair", (0.035, 0.021, 0.015), 0.58)
shirt = material("marigold blouse", (0.95, 0.55, 0.06))
dress = material("burnt orange dress", (0.72, 0.19, 0.035))
...
for side in [-1, 1]:
    ...
    box("shoe strap", (x, -0.015, 0.124), (0.16, 0.04, 0.016), dress, 0.006)
ellipsoid("blouse", (0, 0, 0.76), (0.225, 0.15, 0.21), shirt)
...
ellipsoid("porcelain face", (0, -0.068, 1.2), (0.263, 0.23, 0.272), skin)
# One continuous cap gives the reference's straight fringe, without bead-like locks.
vertices, faces = [], []
for row in range(17):
    for col in range(48):
        phi = col * math.tau / 48
        front = max(0, -math.sin(phi))
        fringe = min(1, max(0, (front - 0.2) / 0.5))
        theta = (0.005 + row / 16 * (2.08 - fringe * 0.81))
        vertices.append((0.315 * math.sin(theta) * math.cos(phi),
                         0.32 * math.sin(theta) * math.sin(phi),
                         1.245 + 0.31 * math.cos(theta)))
for row in range(16):
    for col in range(48):
        a, b = row * 48 + col, row * 48 + (col + 1) % 48
        faces.append((a, a + 48, b + 48, b))
mesh = bpy.data.meshes.new("continuous straight fringe")
mesh.from_pydata(vertices, [], faces)
obj = bpy.data.objects.new("continuous straight fringe", mesh)
bpy.context.collection.objects.link(obj)
for face in mesh.polygons:
    face.use_smooth = True
finish(obj, "continuous straight fringe", hair)
for side in [-1, 1]:
    ...
    ellipsoid("almond eye", (side * 0.096, -0.277, 1.232), (0.045, 0.013, 0.019), cream)
    ellipsoid("brown iris", (side * 0.096, -0.29, 1.231), (0.016, 0.006, 0.017), iris)
    ellipsoid("pupil", (side * 0.096, -0.296, 1.231), (0.009, 0.004, 0.013), dark)
    ellipsoid("eye glint", (side * 0.096 - 0.005, -0.3, 1.238), (0.004, 0.003, 0.004), cream)
    limb("upper eyelid inner", (side * 0.053, -0.281, 1.235), (side * 0.094, -0.29, 1.252), 0.0035, hair)
    limb("upper eyelid outer", (side * 0.094, -0.29, 1.252), (side * 0.141, -0.269, 1.238), 0.0035, hair)
...
for kind in ["guard", "player"]:
    reset()
    is_guard = kind == "guard"
    suit = material(f"{kind} fabric", (0.65, 0.018, 0.15) if is_guard else (0.012, 0.23, 0.19))
    seam = material(f"{kind} fabric seams", (0.27, 0.007, 0.055) if is_guard else (0.008, 0.12, 0.095))
    ...
    def oval(name, pos, scale, mat):
        return ellipsoid(name, pos, scale, mat, 20 if is_guard else 12, 12 if is_guard else 8)

    oval("jacket torso", (0, 0, 0.69), (0.275, 0.212, 0.28), suit)
    limb("waist band", (0, 0, 0.45), (0, 0, 0.51), 0.213, seam)
    for side in [-1, 1]:
        x = side * 0.12
        ...
        oval("shoulder", (side * 0.27, 0, 0.79), (0.099, 0.117, 0.137), suit)
        if is_guard:
            ...
            arm = oval("sleeve", (side * 0.31, 0, 0.66), (0.089, 0.1, 0.19), suit)
            arm.rotation_euler.y = side * 0.13
            limb("wrist cuff", (side * 0.33, 0, 0.46), (side * 0.325, 0, 0.52), 0.08, seam)
            oval("hand", (side * 0.335, -0.008, 0.435), (0.077, 0.062, 0.081), skin)
        if not is_guard:
            box("trouser white stripe", (side * 0.213, 0, 0.28), (0.012, 0.034, 0.27), stripe, 0.005)
            box("sleeve white stripe", (side * 0.382, -0.005, 0.67), (0.012, 0.035, 0.22), stripe, 0.005)
            for lace in range(3):
                box("shoe lace", (x, -0.094 + lace * 0.026, 0.143), (0.1, 0.01, 0.009), seam, 0.003)
    box("center zipper", (0, -0.212, 0.71), (0.012, 0.012, 0.36), rubber if is_guard else stripe, 0.003)
    ...
    if is_guard:
        ...
        oval("head", (0, -0.065, 1.18), (0.265, 0.226, 0.275), skin)
        oval("hair back", (0, 0.035, 1.215), (0.283, 0.245, 0.283), hair)
        for i in range(7):
            lock = oval("swept hair lock", ((i - 3) * 0.071, -0.145, 1.386 - abs(i - 3) * 0.021),
                        (0.073, 0.112, 0.105), hair)
            lock.rotation_euler.y = -0.3
        for side in [-1, 1]:
            oval("ear", (side * 0.265, -0.025, 1.175), (0.043, 0.047, 0.063), skin)
            oval("eye", (side * 0.096, -0.277, 1.203), (0.026, 0.011, 0.032), dark)
            oval("eye highlight", (side * 0.096 - 0.007, -0.288, 1.213), (0.007, 0.004, 0.008), stripe)
            collar = box("jacket collar", (side * 0.06, -0.142, 0.91), (0.1, 0.048, 0.073), stripe, 0.012)
            collar.rotation_euler.y = side * 0.3
        oval("nose", (0, -0.29, 1.15), (0.022, 0.025, 0.027), skin)
        oval("mouth", (0, -0.279, 1.08), (0.028, 0.004, 0.005), rose)
        box("chest badge", (0.126, -0.2, 0.77), (0.093, 0.015, 0.055), stripe, 0.005)
    export(kind)
...
def branch(start, end, radius, depth):
    limb("tapered branch", start, end, radius, bark, radius * 0.57)
    ellipsoid("branch joint", end, (radius * 0.57,) * 3, bark)
    if depth:
        for side in [-1, 1]:
            delta = Vector(end) - Vector(start)
            angle = side * rng.uniform(0.3, 0.7)
            dx = delta.x * math.cos(angle) - delta.z * math.sin(angle)
            dz = delta.x * math.sin(angle) + delta.z * math.cos(angle)
            next_end = Vector(end) + Vector((dx, delta.y + rng.uniform(-0.5, 0.5), dz)) * 0.68
            branch(end, next_end, radius * 0.55, depth - 1)


branch((0, 0, 0.1), (0.15, 0, 3.5), 0.65, 0)
for end, start, radius, depth in [((-0.4, 0.2, 5.8), (0.15, 0, 3.5), 0.4, 4),
                                  ((-2, -0.1, 4.5), (0.1, 0, 2.7), 0.32, 4),
                                  ((2.8, 0.5, 5.3), (0.15, 0, 3.5), 0.3, 4)]:
    branch(start, end, radius, depth)
for i in range(9):
    a = i * math.tau / 9
    limb("spreading root", (math.cos(a) * 1.4, math.sin(a), 0.05), (0, 0, 0.7), 0.07, bark, 0.22)
for i in range(15):
    a = i * math.tau / 15
...
reset()
plaster = material("warm plaster", (0.78, 0.72, 0.53))
trim = material("limestone trim", (0.9, 0.83, 0.65))
wood = material("sage doors", (0.12, 0.23, 0.16))
glass = material("dark teal glass", (0.055, 0.13, 0.13), 0.24)
roof = material("terracotta", (0.42, 0.105, 0.042))
tile = material("sunlit terracotta", (0.57, 0.18, 0.072))
