...
export function createCharacter(kind: "player" | "doll" | "guard" = "player"): THREE.Group {
  ...
  const suit = kind === "doll" ? 0xeaa31c : kind === "guard" ? 0xb72f5c : 0x176659;
  const skin = 0xe6b78b;
  const hair = 0x26231f;
  sphere(root, suit, 0, 0.65, 0, 0.25, 0.32, 0.18);
  ...
  for (let strand = 0; strand < 7; strand++) {
    ...
    sphere(root, hair, Math.cos(angle) * 0.23, 1.3, -0.09 - Math.sin(angle) * 0.16, 0.12, 0.2, 0.13);
  }
  for (const side of [-1, 1]) {
