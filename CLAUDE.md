...
- **Grace window**: steps arriving within `settings.graceMs` after the ghost enters `LOOKING` return `{ kind: "ignored" }` (no advance, no penalty, `lastFoot` unchanged) — it absorbs venue-audio and phone-network latency. Tests that expect a catch must step at or after `TURNING_TO_LOOK_END + graceMs`.
- **Host-disconnect auto-pause**: `markHostDisconnected(socketId, now)` records the time; `tick()` pauses with `pausedReason: "host-disconnected"` after `HOST_DISCONNECT_PAUSE_MS` (music only plays on the host). Reconnecting does not auto-resume.
- **Batched progress**: `applyStep` records advanced/caught results in a pending map; the tick loop calls `flushProgress()` (`server/src/sockets/broadcast.ts`) to emit one `room:playersProgress` per room per tick (host only in motion mode). The stepping player gets its own result from the `player:step` ack — there is no per-step room broadcast.
- **Rhythm modes**: `GhostAI` takes `settings.rhythmMode`. `classic` must not consume the RNG (keeps seeded E2E timings stable); `fake-out` goes `TURNING_TO_LOOK → TURNING_AWAY` without `LOOKING` and resumes the same cycle's music via `musicOffsetMs`.
